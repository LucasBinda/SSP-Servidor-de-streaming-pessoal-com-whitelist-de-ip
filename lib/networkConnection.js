...
const ALVOS_WAN = [
  ...
  { host: '9.9.9.9', port: 53 },  // Quad9 (fundação suíça sem fins lucrativos, foco em privacidade)
];
...
class NetworkConnection {
  ...
  async aguardarEnderecos(timeoutMs = 3000, intervaloMs = 100) {
  ...
  async verificar() {
    ...
    try {
      ...
      if (alcancavel) {
        const { ipv4, ipv6 } = await this.aguardarEnderecos();
        const linhas = [];
        
        if (ipv4) {
          linhas.push(`IPv4: http://${ipv4.address}:${this.porta}`);
        }

        if (ipv6) {
          linhas.push(`IPv6: http://[${ipv6.address}]:${this.porta}`);
        }
