...
class LogManager {
  ...
  registrarErro(origem, mensagem) {
    ...
    console.error(pintar('vermelho', `\n[${origem}] ${mensagem}`, process.stderr));
  }
  ...
  // Mudança de estado da conectividade de rede (interface caiu / voltou),
  // detectada pela NetworkConnection. Vai pro rede.log E pro terminal — verde
  // quando volta/está no ar, amarelo quando cai. Edge-triggered na origem (só
  // chamado na transição), então aqui não há dedup: cada linha é um evento real.
  rede(online, mensagem) {
    const estado = online ? 'ONLINE' : 'OFFLINE';
    this.registrar('rede.log', `${this.horarioLocal()} - ${estado} - ${mensagem}`);
    if (online) console.log(pintar('verde', `[rede] ${mensagem}`));
    else console.warn(pintar('amarelo', `[rede] ${mensagem}`, process.stderr));
  }
}
