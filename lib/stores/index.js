...
// Intervalo mínimo entre podas OPORTUNISTAS (as penduradas em caminho quente,
// como a carga do catálogo). O boot e o intervalo de 6h ignoram isto e podam
// sempre; este teto só evita que rajadas de requisições repitam a varredura
// inteira. Baixo o bastante pra dado órfão sumir em poucos minutos.
const PODA_OPORTUNISTA_INTERVALO_MS = 2 * 60 * 1000;
...
class Store {
  ...
  // Carimbo (ms) da última varredura completa — base do throttle oportunista.
  static _ultimaPodaMs = 0;
  ...
  // Igual a podarTodas(), mas no MÁXIMO 1x por PODA_OPORTUNISTA_INTERVALO_MS.
  // Feita pra pendurar em caminho quente (cada carga do catálogo) sem repetir a
  // varredura inteira a cada requisição — assim dado órfão (job de arquivo
  // removido, etc.) some em minutos, não nas 6h do intervalo de fundo.
  static podarTodasSeVencido() {
    if (Date.now() - Store._ultimaPodaMs >= PODA_OPORTUNISTA_INTERVALO_MS) {
      Store.podarTodas();
    }
  }
}
