...
// Quando o reencode está LIGADO, o catálogo só LISTA .mp4: o worker converte
// todo o resto pra mp4, então mostrar só mp4 garante que todo card toca em
// qualquer navegador (um não-mp4 aparece assim que a conversão termina).
const CATALOGO_EXTENSOES = ['.mp4'];

// Aplica ao acervo varrido o filtro de extensões que o catálogo aceita AGORA,
// decidido pelo reencodeAtivo do settings.json. Como loadSettings lê o disco a
// CADA chamada (sem cache), trocar a opção vale na hora, SEM reiniciar:
//   reencode LIGADO    -> só CATALOGO_EXTENSOES (.mp4)
//   reencode DESLIGADO -> VIDEO_EXTENSIONS (todos os formatos): sem conversão,
//     filtrar pra mp4 deixaria mkv/avi invisíveis pra sempre, então o catálogo
//     lista tudo que a varredura enxerga.
// A checagem espelha o gate de enfileirarConversoes (if (!reencodeAtivo)) pra o
// filtro do catálogo e o worker concordarem sempre. O worker continua recebendo
// a lista COMPLETA por outro caminho, independente disto.
function filtrarParaCatalogo(arquivos) {
  const extensoes = loadSettings().reencodeAtivo ? CATALOGO_EXTENSOES : VIDEO_EXTENSIONS;
  return arquivos.filter((rel) => extensoes.includes(path.extname(rel).toLowerCase()));
}
...
// Sufixos de "arquivo de controle" que clientes de torrent/download criam
// AO LADO do vídeo (mantendo o nome final) enquanto ele ainda baixa. Se um
// irmão desses existe, o vídeo está incompleto. Comparados em minúsculas.
const SUFIXOS_INCOMPLETO = ['.!qb', '.aria2', '.part', '.crdownload', '.!ut', '.bc!'];

// Vídeo modificado há menos que isto provavelmente ainda está sendo escrito
// (download in-place que mantém o nome final, sem arquivo de controle — o
// caso de vários clientes de torrent). Deixa o arquivo "assentar" antes de
// catalogar/sondar, senão o ffprobe quebra num container incompleto e o
// worker de reencode marca "failed" (que não re-tenta sozinho).
const QUIESCENCIA_MS = 60 * 1000;

// Um vídeo ainda em download não deve entrar no catálogo (nem na fila de
// reencode / geração de capa) — sondá-lo com ffprobe falha. Dois sinais,
// client-agnósticos: (a) existe um arquivo de controle irmão; (b) o arquivo
// foi tocado agora há pouco. É best-effort — a defesa à prova de falhas é
// baixar fora de media/movies e mover só ao completar.
function arquivoIncompleto(dir, nome, nomesNaPasta) {
  const baseLower = nome.toLowerCase();
  if (SUFIXOS_INCOMPLETO.some((suf) => nomesNaPasta.has(baseLower + suf))) return true;
  try {
    if (Date.now() - fs.statSync(path.join(dir, nome)).mtimeMs < QUIESCENCIA_MS) return true;
  } catch (err) {
    // Sumiu/ilegível entre o readdir e o stat (corrida com o download) — na
    // dúvida, não cataloga; a próxima varredura reavalia.
    return true;
  }
  return false;
}
...
// os que estão dentro de subpastas. Arquivos ainda em download são pulados
// (ver arquivoIncompleto).
function scanMoviesDir(dir = MOVIES_DIR, baseDir = dir) {
  ...
  const nomesNaPasta = new Set(entries.map((e) => e.name.toLowerCase()));
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (arquivoIncompleto(dir, entry.name, nomesNaPasta)) continue;
...
function sincronizarCatalogo(arquivosEncontrados) {
  ...
  // Filtra a lista logo na entrada pelas extensões que o catálogo aceita agora
  // (filtrarParaCatalogo, que depende do reencodeAtivo). Filtrar antes do
  // encontradosSet também faz a remoção de ausentes purgar do catalog.json as
  // entradas de extensão que o catálogo não aceita mais no estado atual.
  arquivosEncontrados = filtrarParaCatalogo(arquivosEncontrados);
  ...
  const listaDesorganizada = [...listaBase, ...novasEntradas];

  const listaAtualizada = ordenarCatalogo(listaDesorganizada);

  const catalogoMudou = JSON.stringify(listaAtualizada) !== JSON.stringify(listaAtual)
...
// ordena pelo nome do aruivo
function ordenarCatalogo(lista) {
  return [...lista].sort((a, b) =>
    (a.arquivo ?? '').localeCompare(
      b.arquivo ?? '',
      'pt-BR',
      {
        numeric: true,
...
const RUIDO_RE = /\b(1080p|2160p|720p|480p|4k|x264|x265|h264|h265|hevc|avc|xvid|bluray|blu-ray|brrip|bdrip|web-?dl|webrip|dl|hdtv|dvdrip|remux|10bit|8bit|dual|dublado|legendado|nacional|5\.1|2\.0|6ch|ac3|eac3|aac|dts|complete|completa|season|temporada)\b/i;
...
function agruparCatalogo(arquivos, overrides = {}) {
  // Mesma regra do sincronizarCatalogo: exibe só as extensões que o catálogo
  // aceita agora (depende do reencodeAtivo — ver filtrarParaCatalogo).
  arquivos = filtrarParaCatalogo(arquivos);
  ...
  entradas.sort((a, b) => {
    const chaveA = a.tipo === 'serie'
      ? a.titulo
      : a.arquivo;

    const chaveB = b.tipo === 'serie'
      ? b.titulo
      : b.arquivo;
