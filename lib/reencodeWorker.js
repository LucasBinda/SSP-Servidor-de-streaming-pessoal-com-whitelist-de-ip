...
// Padronização pra COMPATIBILIDADE de navegador.
//
...
function marcar(relPath, status, detalhe = '') {
  ...
  const anterior = estado.arquivos[relPath] || {};
  const entrada = { status, detalhe, atualizadoEm: new Date().toISOString() };
  // Preserva o contador de tentativas de retry através dos estados
  // intermediários ("processing") — senão cada nova tentativa zeraria a conta
  // e o teto de RETRY_MAX_TENTATIVAS nunca seria atingido.
  if (anterior.tentativas) entrada.tentativas = anterior.tentativas;
  estado.arquivos[relPath] = entrada;
  salvarEstado(estado);
}

// Caminho absoluto do vídeo original a partir do rel usado como chave do estado.
function caminhoOrigem(rel) {
  return path.normalize(path.join(MOVIES_DIR, rel));
}

// --- Retry automático de jobs "failed" -------------------------------------
// Um job que falhou (probe de um container ainda em download, falta de espaço
// momentânea, um .mp4 destino que sumiu depois...) volta a ser tentado sozinho
// depois de RETRY_ATRASO_MS — sem intervenção humana. O ATRASO é o anti-spam:
// um arquivo ainda baixando falha o probe em ~1s, e sem espera viraria um loop
// de CPU. Depois de RETRY_MAX_TENTATIVAS falhas o retry PAUSA (fica "failed")
// pra não insistir eternamente num arquivo que não conserta — MAS retoma
// sozinho se o arquivo mudar no disco (mtime posterior à última tentativa), que
// é justamente o caso do download que finalmente terminou.
const RETRY_ATRASO_MS = 2 * 60 * 1000;
const RETRY_MAX_TENTATIVAS = 5;

// Marca "failed" contando a tentativa (preservada por marcar através do
// "processing"). Avisa uma vez quando o retry pausa num arquivo estático.
function marcarFalha(rel, detalhe) {
  const estado = lerEstado();
  const anterior = estado.arquivos[rel] || {};
  const tentativas = (anterior.tentativas || 0) + 1;
  estado.arquivos[rel] = { status: 'failed', detalhe, tentativas, atualizadoEm: new Date().toISOString() };
  salvarEstado(estado);

  if (tentativas === RETRY_MAX_TENTATIVAS) {
    // Só avisa "pausado" se o arquivo está PARADO — se ainda muda (download em
    // curso), o retry vai continuar sozinho e o aviso seria falso alarme.
    let estatico = true;
    try {
      estatico = fs.statSync(caminhoOrigem(rel)).mtimeMs <= (Date.parse(anterior.atualizadoEm) || 0);
    } catch (err) { /* sumiu -> processar trata como job esquecido */ }
    if (estatico) {
      logManager.aviso('reencode',
        `${rel}: retry automático pausado após ${RETRY_MAX_TENTATIVAS} tentativas — ` +
        `volta a tentar sozinho se o arquivo mudar (ex.: download concluir), ou remova a entrada de data/reencode-state.json`);
    }
  }
}

// Um "failed" está pronto pra nova tentativa quando (a) já passou o atraso
// anti-spam desde a última, (b) o arquivo ASSENTOU (não é escrito há um tempo —
// senão a tentativa só falharia e poluiria o log durante todo o download) e
// (c) ainda tem tentativas no teto OU o arquivo mudou desde a última tentativa
// (download que terminou -> vale reprocessar mesmo tendo estourado o teto).
function falhaDeveTentar(rel, info) {
  const desde = Date.parse(info.atualizadoEm) || 0;
  if (Date.now() - desde < RETRY_ATRASO_MS) return false;
  let mtimeMs;
  try {
    mtimeMs = fs.statSync(caminhoOrigem(rel)).mtimeMs;
  } catch (err) {
    return false; // arquivo sumiu — processar() trata como job esquecido
  }
  if (Date.now() - mtimeMs < RETRY_ATRASO_MS) return false; // ainda sendo escrito
  return (info.tentativas || 0) < RETRY_MAX_TENTATIVAS || mtimeMs > desde;
}

// Heartbeat do retry: varre o estado e re-enfileira os "failed" já elegíveis.
// Enfileira direto do estado (não do scan de disco), então independe de alguém
// abrir o catálogo — é o que torna a recuperação realmente automática.
function retentarFalhas() {
  const estado = lerEstado();
  const prontos = Object.keys(estado.arquivos).filter((rel) => {
    const info = estado.arquivos[rel];
    return info.status === 'failed' && falhaDeveTentar(rel, info);
  });
  if (prontos.length) enfileirarConversoes(prontos);
}
...
function prepararWorker() {
  ...
  // Heartbeat do retry automático de jobs "failed" (ver retentarFalhas). Roda
  // por toda a vida do processo; unref() pra não segurar o node no shutdown.
  setInterval(retentarFalhas, RETRY_ATRASO_MS).unref();
}
...
function enfileirarConversoes(relPaths) {
  ...
  for (const rel of relPaths) {
    ...
    // "done" já foi resolvido (convertido, ou constatado já-compatível) e
    // "processing" é o job atual — nunca voltam. "failed" volta APENAS quando o
    // retry automático julgar que já pode tentar de novo (ver falhaDeveTentar).
    if (info && info.status !== 'pending') {
      if (!(info.status === 'failed' && falhaDeveTentar(rel, info))) continue;
    }
...
function processarProximo() {
  ...
  // Log instantâneo ao pegar o próximo da fila: sai imediatamente após o
  // "concluído" do anterior, sem o silêncio da sondagem/prova de velocidade
  // que vem antes do "iniciando". Deixa visível que a fila está andando.
  const restam = fila.length ? ` · ${fila.length} na fila` : '';
  logManager.info('reencode', ` próximo: ${rel}${restam} — sondando faixas…`);

  processar(rel)
    .catch((err) => {
      logManager.registrarErro('reencode', `erro inesperado em ${rel}: ${err.message}`);
      marcarFalha(rel, err.message);
    })
    .finally(() => {
      processando = false;
      processarProximo();
...
async function processar(rel) {
  ...
  if (!fs.existsSync(origem)) {
    ...
    salvarEstado(estado);
    logManager.info('reencode', `  ${rel} sumiu antes de processar — job esquecido`);
    return;
  }
  ...
  if (ehMp4 && videoOk && audioOk) {
    ...
    logManager.info('reencode', `  ${rel} já compatível (mp4/h264 8-bit/aac estéreo) — nada a fazer`);
    return;
  }
  ...
  if (!mesmoArquivo && fs.existsSync(destino)) {
    marcarFalha(rel, `já existe um .mp4 com o mesmo nome (${relDestino}) — resolva manualmente`);
  ...
  try {
    ...
    if (livre < tamanhoOriginal * 1.2) {
      marcarFalha(rel, `sem espaço em disco (livre: ${(livre / 1e9).toFixed(1)}GB, necessário: ~${(tamanhoOriginal * 1.2 / 1e9).toFixed(1)}GB)`);
  ...
  // Rótulo curto da tecnologia de encode, pros logs de início/progresso/fim:
  // quando o vídeo é copiado não há encode (só remux); senão é GPU (NVENC[/NVDEC])
  // ou CPU, conforme a prova de velocidade escolheu.
  const tec = videoOk
    ? 'remux (vídeo copiado)'
    : modo === 'nvenc+nvdec' ? 'GPU NVENC+NVDEC'
    : modo === 'nvenc' ? 'GPU NVENC'
    : `CPU ${settings.reencodeCodec}`;
  ...
  logManager.info('reencode', `  iniciando ${rel} · ${tec} · ${resumo}`);

  const inicio = Date.now();
  await rodarFfmpeg(args, tracks.duracao, rel, tec);
  ...
  const minutos = (decorridoS / 60).toFixed(1);
  ...
  // Velocidade média sustentada (segundos de filme por segundo de relógio) —
  // fecha o número que a prova de velocidade só estimou por 1s.
  const velMedia = tracks.duracao > 0 && decorridoS > 0 ? ` @ ${(tracks.duracao / decorridoS).toFixed(1)}x` : '';
  marcar(rel, 'done', `${virou} (${(tamanhoOriginal / 1e9).toFixed(2)}GB -> ${(tamanhoNovo / 1e9).toFixed(2)}GB, ${variacao}, ${minutos}min)`);
  logManager.info('reencode',
    `  concluído ${relDestino} · ${tec} · ${(tamanhoOriginal / 1e6).toFixed(0)}MB → ${(tamanhoNovo / 1e6).toFixed(0)}MB (${variacao}) · faltam ${fila.length} filmes na fila · ${minutos}min${velMedia}`);
}
...
// NVIDIA eles falham em menos de PROVA_MS segundo cada — aí não há por que gastar os
// PROVA_MS da prova do software, que é a única opção restante. Só quando alguma
...
async function escolherModo(origem, codecOrigem, duracaoSegundos, settings, rel) {
  ...
  if (!gpuFuncionou) {
    ...
    // que comparar, então pulamos os PROVA_MS de prova dele e vamos direto.
    logManager.info('reencode', `  prova de velocidade · ${rel} · ${resumoProva(notas)} → CPU (sem GPU, prova pulada)`);
    return 'software';
  }
  ...
  logManager.info('reencode', `  prova de velocidade · ${rel} · ${resumoProva(notas)} → vencedor: ${vencedor.modo}`);
...
// Barrinha ASCII de progresso pro log ("█████░░░░░ 52%") — leitura de relance
// de quanto falta, sem precisar interpretar o número.
function barraProgresso(pct, largura = 10) {
  const cheias = Math.max(0, Math.min(largura, Math.round((pct / 100) * largura)));
  return '█'.repeat(cheias) + '░'.repeat(largura - cheias);
}

function rodarFfmpeg(args, duracaoSegundos, rel, tec = '') {
  return new Promise((resolve, reject) => {
    ...
    // tecnologia (GPU/CPU), a velocidade (1.0x = tempo real) e a estimativa de
    // término — um encode lento com nice -19 avança pouco por minuto e sem isso
    // parece travado. O primeiro update sai já aos ~15s (não espera 1min todo)
    // pra confirmar cedo que está andando; os seguintes, 1x/min.
    let ultimoLog = Date.now() - 10 * 1000;
    let bufferProgresso = '';
    proc.stdout.on('data', (chunk) => {
      bufferProgresso = (bufferProgresso + chunk.toString()).slice(-2000);
      if (Date.now() - ultimoLog < 15 * 1000) return;
      ...
      const pct = (outTimeS / duracaoSegundos) * 100;
      ...
      const prefixoTec = tec ? `${tec} · ` : '';
      logManager.info('reencode', `${rel} · ${prefixoTec}${barraProgresso(pct)} ${pct.toFixed(0)}%${extra}`);
      ultimoLog = Date.now();
    });
